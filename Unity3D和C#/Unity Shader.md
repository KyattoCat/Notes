...
| `float3 Shader4PointLights(...)`               | **仅用于前向渲染**。计算四个点光源的光照，她的参数是已经打包进矢量的光照数据，通常如`unity_4LightPosX0`等，见高级光照附表。前向渲染通常会使用这个函数计算逐顶点光照 |
...
渲染路径是什么？我理解为，我给Unity一个信息，让Unity将一些特定光源的信息存储到内置属性里，我才能正确的调用。如果不指定渲染路径，有可能我们的计算结果就是错误的（因为用到的属性是错误的）。LightMode标签用于设置渲染路径，具体见附表。
...
前向渲染可以使用的内置光照变量见附表，同时还可以回头看一下基础光照那一节的附表仅用于前向渲染的函数。
