...
空间中的物体对应的z坐标去除，获得的图像就是正交投影下的图，前提是该物体在相机的剪裁平面。之后再将物体的x和y坐标映射到[-1, 1]，这是约定俗成的行为，为的是之后的计算更方便。

还有一种方法更加“正式”。首先定义一个长方体，左l右r下b上t远f近n。这个长方体就是相机所拍摄到的空间。之后将这个长方体映射到规范立方体（每个坐标都在[-1, 1]内），映射方法先平移后缩放。变换矩阵如下：

![](images/计算机图形学/正交投影.png)

##### 2.2.3.2 透视投影

在继续之前，说明一下，在齐次坐标中，(x, y, z, 1)、(kx, ky, kz, k!=0)、**(xz, yz, z^2, z!=0)**表示的都是一个同一个三维点(x, y, z)，下面会用到。

正交投影时，我们定义了一个空间中的长方体，并将其映射到规范立方体。透视投影是从一个点，向四棱锥的四条棱发射，同时定义一个近平面和远平面，这样就截出来一个四棱台（是叫这个吗，还是看图吧）。

![](images/计算机图形学/透视投影选取区域压缩.png)

想象一下，我们可以先把远平面的四个点挤压到和近平面同一高度上，或者说，把这个四棱台压缩成一个立方体，就像上图的左右。这个挤压的过程中，近平面保持不变，物体的在近平面和远平面z坐标不变，远平面中心点不变。然后我们再对这个立方体做一次正交投影的操作，就可以实现透视投影。正交投影已经有变换矩阵了，问题就是如何挤压。

![](images/计算机图形学/透视投影1.png)

我们先求一个坐标的，如上图右侧所示，我们从侧面观察，取得一个中心处的上半部分的截面，如上图左所示。基于相似三角形可以得出$y^\prime=\frac{n}{z}y$。类似的，我们从上方观察，可以得出$x^\prime=\frac{n}{z}x$。我们还不清楚z坐标如何变换，因为我们只确定在近平面和远平面的物体z坐标不变，但是我们可以先写出来一个关系：
$$
M_{persp\rightarrow ortho}\left[\matrix{x\\y\\z\\1}\right]=\left[\matrix{\frac{nx}{z}\\\frac{ny}{z}\\?\\1}\right]=\left[\matrix{nx\\ny\\?\\z}\right]
$$
根据上面的结果可以推导出部分的变换矩阵：
$$
M_{persp\rightarrow ortho}=\left[\matrix{n&0&0&0\\0&n&0&0\\?&?&?&?\\0&0&1&0}\right]
$$
利用近平面和远平面z坐标不变，可以将在近平面和远平面的点带入到上面的计算中：
$$
M_{persp\rightarrow ortho}\left[\matrix{x\\y\\n\\1}\right]=\left[\matrix{nx\\ny\\n^2\\n}\right]
$$
第三个数是$n^2$，说明计算与x和y不相关，但是仍然不知道后两个数是否对结果产生影响，所以设后两个数为A和B：
$$
\left[\matrix{0&0&A&B}\right]\left[\matrix{x\\y\\n\\1}\right]=n^2
$$
又有远平面上中点变换后坐标不变：
$$
M_{persp\rightarrow ortho}\left[\matrix{0\\0\\f\\1}\right]=\left[\matrix{0\\0\\f^2\\f}\right]
$$
可得：
$$
An+B=n^2\\
...
思考一个问题，对于在这个四棱台内的任意一个点(x, y, z)，在进行投影-正交矩阵的变换之后，这个点的z坐标会更靠近近平面还是远平面？或者简单一点，对于一个点(x, y, (n+f)/2)，即这个点在近平面和远平面的中心平面，这个点被压缩后，她的z坐标如何变化？

答：直接代入(x, y, z, 1)进行矩阵运算，得到最终的点为(nx/z, ny/z, n+f-nf/z, 1)，而n+f-nf/z是一个反比例函数，且恒过(n, n)和(f, f)，在[f, n]范围内是一个凹函数，所以z坐标的值会变小，也就是会更加靠近远平面。假设n为-1，f为-2，其函数图像如下：

![](images/计算机图形学/投影变换思考体函数图像.png)

----

定义近平面还可以用另外的参数定义：

1. vertical fov（field of view），确定了相机在垂直角度的观察角度
2. aspect ratio，把近平面用宽度和高度表达，用宽度除以高度得到宽高比

![](images/计算机图形学/透视投影近平面另一种定义.png)

之前的表达方式和这个表达方式可以互相转换，具体转换过程不写了，照着之前的推导图，用fov角度和宽高比重新推一下就可以。

## 3. 光栅化

### 3.1 视口变换

前面的变换章节已经完成了MVP变换，将物体放在了[-1, 1]的规范立方体中。之后的操作就是将这个规范立方体给显示到屏幕上，这就是光栅化（Raster）。

> Raster == screen in German
>
> Rasterize == drawing onto the screen

首先定义一下在这次学习中的屏幕，在图形学中，屏幕是一个像素数组，数组的大小就是所谓分辨率，比如1920x1080、1280x720等。像素是一个内部颜色完全相同小方块（即像素是显示的最小单位），像素的颜色是RGB的混合，我们认为像素的坐标在像素的左下角，像素的中点为像素坐标+0.5。屏幕的左下角为原点，向右为x轴正方向，向上为y轴正方向。

我们要将一个规范立方体转换到屏幕上，就会有一个问题，z轴怎么办？按下不表，先讨论xy的变换。

我们要将[-1, 1]映射到[0, width]和[0, height]，只需要缩放后平移：
$$
M_{viewport}=\left[\matrix{width/2&0&0&width/2\\0&height/2&0&height/2\\0&0&1&0\\0&0&0&1}\right]
$$
这个变换矩阵称为视口变换矩阵。

现代显示器的显示方式大概是把显存中的一块区域映射到屏幕上，Frame Buffer帧缓存。

### 3.2 多边形

三角形是最基础的多边形，任何其他不同的多边形都可以拆分成三角形。三角形一定是平面的，三角形内外定义十分清晰（可以回头看一下向量的叉乘）。只要给出三角形三个顶点的属性，三角形内部的属性是可以通过插值获得的。一般模型都是由三角面片构成的。

光栅化时，最重要的就是判断一个像素的中心点和三角形的内外关系，我们通过采样获取一个像素是否在三角形内，判断是否在三角形内的函数我们定义如下：
$$
inside(t, x, y)=\left\{\matrix{1,\space Point\space(x, y)\space in\space triangle\space t\\0,\space otherwise}\right.
$$

![](images/计算机图形学/光栅化采样.png)
...
遍历的范围不可能是全部的像素，而是使用一个包围盒进行规定的。包围盒的大小和起始位置取决于三角形的顶点坐标，如下图所示：

![](images/计算机图形学/采样包围盒.png)

这个包围盒的正式名称为轴向包围盒（Axis-Aligned Bounding Box，缩写AABB）。课程上也提到一个适用于三角形实际不大，但包围盒很大的情况下的遍历方法，Incremental Triangle Traversal。

inside函数内部实现伪代码如下，自己写的，可能有错：

```c
int inside(Triangle tri, float x, float y)
{
    // 定义Triangle中用一个大小为3的数组表示顶点坐标，且按照逆时针排布
    for (int i = 0; i < 3; i++)
    {
        vec3 tri_border_vec = tri.vertex[i] - tri.vertex[(i + 1) % 3];
        vec3 vertex_point_vec = tri.vertex[i] - vec2(x, y);
        vec3 cross_result = cross(normalize(tri_border_vec), normalize(vertex_point_vec));
        if (cross_result == vec3.zero) return 1;
        if (cross_result == vec3.forward) return 0;
    }
    return 1;
}
```

点落在三角形边界的情况可以按照自己的规定来处理，我在上面是保留了边界上的点的。

当我们确定了哪些点是在三角形内后，我们就可以绘图了。这时候就有一个问题随之而生：走样（锯齿）。原因是像素本身具有一定的大小，如果我们按照计算出来的点绘制的话，就会如下图所示：

![](images/计算机图形学/采样走样.png)

这与我们期待的三角形其实有一定差距，虽然大体形状相似。这是光栅化中一直在解决的严重问题。

![](images/计算机图形学/采样走样2.png)

### 3.3 反走样（抗锯齿）

采样出现问题的根本原因是，信号变化过快，采样速度跟不上变化速度。如下图所示，假设我们有两个周期函数，分别以蓝色和黑色表示其图像，同时以某个间隔进行采样，可以发现，采样两个频率完全不同的函数得到的结果却是一样的，这就是走样：

![](images/计算机图形学/走样.png)
